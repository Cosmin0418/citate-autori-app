...
export default function QuotesPage() {
  // Stările în care se poate afla pagina
  const [quotes, setQuotes] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // Când se deschide pagina, cere citatele de la server
  useEffect(() => {
    getAllQuotes()
  ...
  // Dacă încă se încarcă, arată un mesaj
  if (loading) {
    return (
  ...
  // Dacă a apărut o eroare la conexiune, arată eroarea cu roșu
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-brand-light">
    ...
    <div className="min-h-screen bg-brand-light">
      ...
      <header className="sticky top-0 z-10 bg-white shadow-sm">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          {/* Butonul spre pagina de administrare */}
          <Link
            to="/manage"
